...
void DistLNF_B::drawLinearSlider(juce::Graphics& g, int x, int y, int width, int height, float sliderPos, float minSliderPos, float maxSliderPos, const juce::Slider::SliderStyle style, juce::Slider& slider)
{
...
//            float triangleY = 0.0f;
            ...
            float fillHeight = triangleHeight * normalizedValue;
            float phillY = baseY - fillHeight;
            float totalHalfBase = triangleWidth / 2.0f;
            float currentHalfBase = (fillHeight / triangleHeight) * totalHalfBase;

            // Inner fill
            juce::Path fillTriangle;
            fillTriangle.addTriangle(topX, topY, topX - currentHalfBase, phillY + fillHeight, topX + currentHalfBase, phillY + fillHeight);
            g.setColour(juce::Colour (0xffedf2f4));
